...
def make_data(expe,n_samples,dim=2,device='cpu'):
    if expe=='spirals':
        ...
        xs = make_blobs_reg(n_samples=n_samples, n_blobs=3)
        xt = make_blobs_reg(n_samples=n_samples, n_blobs=3).dot(get_rot(r))
        xs[:int(n_samples/3),:]=xs[:int(n_samples/3),:]+np.array([2,2])
        ...
        xs = np.random.rand(n_samples, 2) - .5
        xs = xs + np.sign(xs)
        xt = np.random.rand(n_samples, 2) - .5
    elif expe=='gaussians' :
        mu_s=np.ones(dim)
        cov_s=np.ones(dim)
...
def make_data_1d(expe,n_samples,device='cpu'):
    ...
    if expe=="cluster":
        xs=np.concatenate((np.random.normal(-10, 1, 5*n_samples),np.random.normal(0, 1, 3*n_samples),np.random.normal(20, .8, n_samples)))
        xt=np.concatenate((np.random.normal(-10, 1, 3*n_samples),np.random.normal(0, 1, int(5.5*n_samples))))
